...
#include "stdlib.h" 
#include "stdio.h"
#define MAX_SEG 50  //number of segments
#define STATIC_MEM_SIZE 2*1024*1024 //size in bits of each segment
#define QUEUE_SIZE 12*1024*1024  //intermediate quantity
...
void* m_malloc(unsigned int size)  //allocates sdram
{
	int i;
...
void m_free(void* p)  //frees up sdram
{
	int i;
	if(mem_count <= 1)
	{
